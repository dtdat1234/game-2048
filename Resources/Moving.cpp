...
bool isMoveUp(GamePlay2048 game) {
    ...
    for (int col = 0; col < n; col++) {
        for (int row = 0; row < n - 1; row++) {
            if (game.boardGame[row][col] != 0) { //If the current cell is non-zero
            ...
            else if (game.boardGame[row][col] == 0) { //If the current cell is zero
                int pos = row + 1;
                while (pos < n && game.boardGame[pos][col] == 0) { //Find position where the cell has non-zero value
                    pos++;
                }
                if (pos < n) { //If a non-zero value is found
                    return true;
                }
            }
        }
...
void ProcessUp(GamePlay2048& game) {
    ...
    for (int col = 0; col < n; col++) {
        for (int row = 0; row < n - 1; row++) {
            ...
            if (pos < n) { // If a non-zero value is found
                if (game.boardGame[row][col] != 0) { // If the current cell is not zero
                    ...
                    if (pos < n) { // If another non-zero value is found (similar to previous search)
                        if (game.boardGame[pos][col] == game.boardGame[row][col]) {
                            game.boardGame[row][col] *= 2; // Combines the values
                            game.boardGame[pos][col] = 0; 
                            game.score += game.boardGame[row][col]; // Calculate score
    ...
    for (int col = 0; col < n; col++) {
        for (int row = n - 1; row > 0; row--) {
            if (game.boardGame[row][col] != 0) { //If the current cell is not zero
                ...
                int pos = row - 1;
                while (pos >= 0 && game.boardGame[pos][col] == 0) { //Find position where the cell has non-zero value
                    pos--;
                }
                if (pos >= 0) {//If a non-zero value is found
                    return true;
                }
            }
        }
...
void ProcessDown(GamePlay2048& game) {
    ...
    for (int col = 0; col < n; col++) {
        for (int row = n - 1; row >= 0; row--) {
            ...
            if (pos >= 0) {
                if (game.boardGame[row][col] != 0) {// If a non-zero value is found
                    //Check if the current cell and the found cell has the same value
                    if (game.boardGame[pos][col] == game.boardGame[row][col]) {
                        game.boardGame[row][col] *= 2;// Combines the values
                        game.boardGame[pos][col] = 0;
                    ...
                    if (pos >= 0) { // If another non-zero value is found (similar to previous search)
                        if (game.boardGame[pos][col] == game.boardGame[row][col]) {
                            game.boardGame[row][col] *= 2;// Combines the values
                            game.boardGame[pos][col] = 0;
                            game.score += game.boardGame[row][col];
    ...
    for (int row = 0; row < n; row++) {
        for (int col = 0; col < n - 1; col++) {
            if (game.boardGame[row][col] != 0) { //If the current cell is non-zero
                ...
                int pos = col + 1;
                while (pos < n && game.boardGame[row][pos] == 0) {//Find position where the cell has non-zero value
                    pos++; 
                }
                if (pos < n) {//If a non-zero value is found
                    return true;
    ...
    for (int row = 0; row < n; row++) {
        for (int col = n - 1; col > 0; col--) {
            if (game.boardGame[row][col] != 0) {//If the current cell is non-zero
                ...
                int pos = col - 1;
                while (pos >= 0 && game.boardGame[row][pos] == 0) {//Find position where the cell has non-zero value
                    pos--;
                }
                if (pos >= 0) {//If a non-zero value is found
                    return true;
